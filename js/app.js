    ...
    // ===== 音源切换性能优化：缓存层 =====
    var _fallbackSearchCache = new Map(); // key: title|artist|source -> { tracks, limit, expireAt }
    ...
    function _getCachedFallbackSearch(track, source, limit) {
    ...
    function _setCachedFallbackSearch(track, source, tracks, limit) {
    ...
    async function resolveFallbackTrackFromSource(track, source, skipUrls, limit) {
      ...
      var matches = typeof _getCachedFallbackSearch === 'function' ? _getCachedFallbackSearch(track, source, searchLimit) : null;
      if (!matches) {
        ...
        if (typeof _setCachedFallbackSearch === 'function') _setCachedFallbackSearch(track, source, matches, searchLimit);
      }
