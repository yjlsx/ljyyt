    ...
    const sourceDisplayOrder = ['local', 'joox', 'qq', 'netease', 'kuwo', 'migu', 'bilibili', '_netease'];
    ...
    const defaultEnabledSources = ['local', 'joox', 'qq', 'netease', 'kuwo', 'bilibili'];
    ...
    function isTrackMatchCandidate(target, candidate) {
      ...
      var simpleCandidateTitle = normalizeTrackText(String(candidate && candidate.title || '').replace(/[（(].*?[）)]/g, ''));
    ...
    function isLooseTitleMatchCandidate(target, candidate) {
      ...
      var candidateTitle = normalizeTrackText(candidate && candidate.title);
      var simpleCandidateTitle = normalizeTrackText(String(candidate && candidate.title || '').replace(/[（(].*?[）)]/g, ''));
      return !!(title && candidateTitle && (title === candidateTitle || title === simpleCandidateTitle));
    }
    ...
    function getNormalizedArtistTokens(value) {
      return normalizeTrackText(value)
        .split(/(?:feat|featuring|ft|with|和|与|及|\/|、|,|，|&|\+)+/i)
    ...
    async function resolveFallbackTrackFromSource(track, source, skipUrls) {
      ...
      var matches = await fetchExternalSourceTracks(fallbackQuery, source, SEARCH_RESULT_LIMIT);
      if (!matches.length && fallbackArtistTokens.length) {
        matches = await fetchExternalSourceTracks(track.title, source, SEARCH_RESULT_LIMIT);
      }
      var candidates = getFallbackTrackMatches(track, matches);
      for (var i = 0; i < candidates.length; i++) {
        var match = candidates[i];
        var url = match.src || await resolveExternalTrackUrl(match);
        if (!url || skipUrls.indexOf(String(url).trim()) >= 0) continue;
    ...
    async function recoverPlayableTrackUrl(track, options) {
      ...
      try {
        var result = await Promise.any(sources.map(function(source) {
          return resolveFallbackTrackFromSource(track, source, skipUrls).then(function(found) {
    ...
    async function consumeFallbackPrewarm(track, state, fallbackKey) {
      ...
      if (source && state.sources.indexOf(source) >= 0) return null;
      if (url && state.urls.indexOf(url) >= 0) return null;
      return result;
    }
    ...
    async function playAudioWithTimeout(timeoutMs) {
    ...
    async function fetchOtterNetease(path, payload) {
      ...
      throw lastError || new Error('otter netease api failed');
    }
    ...
    async function fetchQqSearchPage(query, page, count, signal) {
      ...
      try {
        ...
        console.warn('QQ primary search failed, using fallback proxy', error);
      }
      ...
      if (!fallbackResponse.ok) throw new Error('QQ fallback search ' + fallbackResponse.status);
      var fallbackPayload = await fallbackResponse.json();
      var items = fallbackPayload && Array.isArray(fallbackPayload.items) ? fallbackPayload.items : [];
      return items.slice(0, count).map(normalizeQqProxyTrack).filter(Boolean);
    }
    ...
    async function resolveExternalTrackUrl(track) {
      ...
      if (track.source === 'qq') {
        try {
          var qqPayload = await fetchQqTrackUrlPayload(track.urlId);
