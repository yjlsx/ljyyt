      ...
      for (var attempt = 0; attempt < attemptLimit; attempt++) {
        ...
        try {
          ...
          if (!fallbackUrl) {
            ...
            if (!fallbackUrl) {
              var normalizedFallbackTitle = typeof normalizeTrackText === 'function'
                ? normalizeTrackText(currentTrack && currentTrack.title)
                : String(currentTrack && currentTrack.title || '').trim().toLowerCase();
              if (normalizedFallbackTitle && normalizedFallbackTitle.length <= 4) {
                fallbackUrl = await recoverPlayableTrackUrl(currentTrack, {
                  skipSources: state.sources.slice(),
                  skipUrls: state.urls.slice(),
