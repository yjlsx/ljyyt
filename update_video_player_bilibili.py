...
# 查找 renderVideoPlayer 函数
render_video_player_pattern = r'(function renderVideoPlayer\(video\) \{[^}]*var container = document\.getElementById\(\'videoContainer\'\);[^}]*var info = document\.getElementById\(\'videoInfo\);\s*if \(!video\) \{[^}]+\}[^}]*// 创建视频元素\s*var videoElement = document\.createElement\(\'video\'\);)'

match = re.search(render_video_player_pattern, content, re.DOTALL)
if match:
    print('✅ 找到 renderVideoPlayer 函数')
    
    # 替换整个视频播放逻辑
    old_video_logic = r"""// 创建视频元素
    ...
    new_video_logic = r"""// 检查是否是Bilibili视频
      ...
      if (isBilibiliVideo) {
        ...
        console.log('🎬ari 普通视频，使用HTML5视频播放器');
    ...
    content = re.sub(old_video_logic, new_video_logic, content, flags=re.DOTALL)
    print('✅ 已更新视频播放逻辑')
    print('   - 添加了Bilibili视频识别')
    print('   - 添加了Bilibili嵌入播放器')
    print('   - 保留了普通视频播放器')
else:
    print('❌ 未找到 renderVideoPlayer 函数')
